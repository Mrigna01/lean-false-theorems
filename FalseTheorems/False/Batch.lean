...
/-!
100 intentionally false (refutable) theorem statements.
They are written to compile (via `by sorry`) so LeanDojo can trace them.
-/
...
-- ===== Prop / logic (76–100) — refutable by choosing P := False or P := True =====
...
theorem false_086 : (∀ P Q : Prop, (P → Q) ∧ (Q → P) → (P = Q)) := by sorry
